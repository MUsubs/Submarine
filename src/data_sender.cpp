...
DataSender::DataSender( int led_pin, int frequency, int msg_queue_length, int task_priority ) :
    _byte_queue( xQueueCreate( msg_queue_length, sizeof( uint8_t ) ) ),
    _unit_us( 1'000'000 / frequency ), _led_pin( led_pin ) {
...
uint8_t DataSender::generateInstructionHeader( inst_t inst, uint8_t n_bytes ) {
    ...
    return ( static_cast<int>( packet_t::INST ) << 6 ) | ( static_cast<int>( inst ) << 3 ) |
           n_bytes;
}

uint8_t DataSender::generateUpdateHeader( data_t data_id, uint8_t n_bytes ) {
    if ( n_bytes > 0b111 ) return 0;
    return ( static_cast<int>( packet_t::UPDATE ) << 6 ) | ( static_cast<int>( data_id ) << 3 ) |
           n_bytes;
}

uint8_t DataSender::generateSensorHeader( sens_t sensor, uint8_t n_bytes ) {
    if ( n_bytes > 0b1111 ) return 0;
    return ( static_cast<int>( packet_t::SENS ) << 6 ) | ( static_cast<int>( sensor ) << 4 ) |
           n_bytes;
}
